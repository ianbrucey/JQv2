  ...
  const handleCreateDraft = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.detail || `Failed to create draft: ${response.status}`);
      }
    ...
    return (
      <div className="p-6 text-center">
        <div className="text-gray-500">
          ...
          <p className="text-sm mt-2">Drafts are only available for legal case conversations.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="p-6 border-b border-neutral-600">
        ...
        {drafts.length === 0 && !loading && (
          <div className="bg-neutral-800 rounded-lg p-6">
            <h3 className="text-lg font-medium mb-4">Create Your First Draft</h3>
            <form onSubmit={handleCreateDraft} className="space-y-4">
              <div>
                <label htmlFor="draft-type" className="block text-sm font-medium mb-2">
                  Draft Type
                </label>
                <select
                  id="draft-type"
              ...
              <div>
                <label htmlFor="draft-name" className="block text-sm font-medium mb-2">
                  Draft Name
                </label>
                <input
                  id="draft-name"
        ...
        {!loading && drafts.length > 0 && (
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              <h3 className="text-lg font-medium">Your Drafts ({drafts.length})</h3>
              <button
                onClick={() => {
                  // TODO: Implement create new draft when drafts exist
                  setDraftName("");
                  setDraftType("complaint");
                }}
